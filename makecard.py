...
def main():
  ...
  parser.add_argument("-s"  , "--tag"     , nargs='+', type=str)
  ...
  for n, p in datasets.items():
    ...
    name = "VBS" if p.ptype=="signal" else p.name
    if p.ptype=="data":
      continue
    ...
    card.add_shape_nuisance(name, "CMS_EFF_e", p.get("ElecronSF" ), symmetrise=False)
    card.add_shape_nuisance(name, "CMS_EFF_m", p.get("MuonSF")    , symmetrise=False)
    ...
    card.add_shape_nuisance(name, "CMS_Trig_{}".format(options.era), p.get("TriggerSFWeight"), symmetrise=False)
    ...
    # if options.era == '2016':
    #   card.add_shape_nuisance(name, "PDF_2016", p.get("PDF"), symmetrise=False)
    # else:
    #   card.add_shape_nuisance(name, "PDF_1718", p.get("PDF"), symmetrise=False)  
    card.add_nuisance(name, "{:<21}  lnN".format("UEPS"),  1.020) # Underlying events
    
    # EWK uncertainties
    if name in ["ZZ"]:
      card.add_shape_nuisance(name, "EWKZZ", p.get("EWK"), symmetrise=False)
    if name in ["WZ"]:
      card.add_shape_nuisance(name, "EWKWZ", p.get("EWK"), symmetrise=False)                          
    
    # define rates
    if name  in ["TOP", "WW"]:
      if "catEM" in card_name:
        card.add_rate_param("EMnorm_" + options.era, "catEM*", name)
      elif "signal" in card_name.lower():
        card.add_rate_param("EMnorm_" + options.era, "signal*", name)

    # define rate 3L category
    elif name in ["WZ"]:
      if ("cat3L" in card_name):
        card.add_rate_param("WZnorm_" + options.era, "cat3L*", name)
      elif "signal" in card_name.lower():
        card.add_rate_param("WZnorm_" + options.era, "signal*", name)

    # define rate for DY category
    elif name in ["DY"]:
      if "DY" in card_name:
        card.add_rate_param("DYnorm_" + options.era, "catDY*", name)
      elif "signal" in card_name.lower():
        card.add_rate_param("DYnorm_" + options.era, "signal*", name)
